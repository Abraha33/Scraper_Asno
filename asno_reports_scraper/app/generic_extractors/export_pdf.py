...
async def inspect_export_pdf(page: Page) -> dict[str, Any]:
    exports = await page.evaluate(
        ...
        })).filter(x => /pdf/i.test(`${x.label} ${x.href || ''}`))"""
    )
    return {"pattern": "export_pdf", "filters": await read_filters(page), "exports": exports, "read_only": True}
